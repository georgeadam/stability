...
class TwoStageKD(LightningModule):
    def __init__(self, model, original_model, new_model, optimizer, lr_scheduler, alpha, preference,
                 annealer, *args, **kwargs):
        super().__init__()
        ...
        self.distill_loss = KDLoss(preference)
    ...
    def _get_loss(self, x, logits, y):
        ...
        original_probs = torch.nn.Softmax(dim=1)(original_logits)
        new_probs = torch.nn.Softmax(dim=1)(new_logits)

        distill_loss = self.distill_loss(logits, original_probs, new_probs, y)
...
class KDLoss(object):
    ...
    def __call__(self, logits, original_probs, new_probs, y_true):
        ...
        new_correct = new_preds == y_true

        if self.preference == "original":
            source_indices = []

            for i in range(len(y_true)):
                if (original_correct[i] and new_correct[i]) or original_correct[i]:
            ...
            for i in range(len(y_true)):
                if (original_correct[i] and new_correct[i]) or new_correct[i]:
                    source_indices.append(1)
                elif original_correct[i]:
                    source_indices.append(0)
                else:
                    source_indices.append(1)
        ...
        sample_indices = torch.arange(len(y_true)).to(y_true.device)
        combined_probs = torch.stack([original_probs, new_probs], dim=1)
        combined_probs = combined_probs[sample_indices, source_indices, :]

        loss = self.ce(logits, combined_probs)

        return loss


lightning_modules.register_builder("two_stage_kd", TwoStageKD)
