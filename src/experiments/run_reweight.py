...
def fit_and_predict_reweight(args, dataset, original_model, logger):
    if args.misc.reset_random_state:
        seed_everything(args.misc.seed)
...
def create_module_reweight(args, model, original_model):
    return lightning_modules.create(args.reweight_module.name, model=model, original_model=original_model,
...
def main(args: DictConfig):
    ...
    wandb_logger = WandbLogger(project="stability", prefix="initial",
                               name="{}_{}_reweight-{}".format(args.data.name,
                                                                   args.model.name,
                                                                   args.misc.seed))
    ...
    new_train_preds, new_test_preds = fit_and_predict_reweight(args, dataset, original_model, wandb_logger)
