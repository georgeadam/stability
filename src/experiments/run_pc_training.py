...
def fit_and_predict_pc(args, dataset, original_model, logger):
    if args.misc.reset_random_state:
        seed_everything(args.misc.seed)

    model = create_model(args, dataset.num_classes)
    module = create_module_pc(args, model, original_model)
    trainer = create_trainer(args, logger)
    trainer.fit(module, datamodule=dataset)
...
def create_module_pc(args, model, original_model):
    return lightning_modules.create(args.pc_module.name, model=model, original_model=original_model,
                                    **args.pc_module.params)
...
def main(args: DictConfig):
    ...
    wandb_logger = WandbLogger(project="stability", prefix="initial",
                               name="{}_{}_pc_training_{}_{}-{}".format(args.data.name,
                                                                        args.model.name,
                                                                        args.pc_module.params.alpha,
                                                                        args.pc_module.params.beta,
                                                                        args.misc.seed))
    ...
    new_test_preds = fit_and_predict_pc(args, dataset, original_model, wandb_logger)
    ...
    # Save predictions
    save_predictions(original_test_preds, new_test_preds)
