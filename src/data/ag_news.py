...
class AGNEWSDataModule(DataModule):
    ...
    def prepare_data(self):
        ...
        if not self.train_data:
            AG_NEWS(self.data_dir, split="train")
            AG_NEWS(self.data_dir, split="test")

    def setup(self, stage: Optional[str] = None):
        if not self.train_data:
            full_data = AG_NEWS(self.data_dir, split="train")
...
def raw_to_processed(data, vocab, tokenizer):
    ...
    for batch in data:
        ...
        sentences.append((torch.tensor(vocab(tokenizer(batch[1])[:300]))))
    ...
    labels = labels - 1  # AG_NEWS labels are 1,2,3,4 convert to 0,1,2,3
    data = TextDataset(sentences, labels)

    return data


datasets.register_builder("ag_news", AGNEWSDataModule)
