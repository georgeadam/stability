...
class FashionMNISTDataModule(DataModule):
    ...
    def prepare_data(self):
        ...
        if not self.train_data:
            MyFashionMNIST(self.data_dir, train=True, download=True)
            MyFashionMNIST(self.data_dir, train=False, download=True)

    def setup(self, stage: Optional[str] = None):
        if not self.train_data:
            full_data = MyFashionMNIST(self.data_dir, train=True)
            ...
            if self.extra_size == 0:
                ...
                train_indices, extra_indices = train_test_split(np.arange(len(train_data)),
            ...
            test_data = MyFashionMNIST(self.data_dir, train=False)
            self.test_data = AugmentedDataset(test_data, np.arange(len(test_data)), 0)
            predict_data = MyFashionMNIST(self.data_dir, train=False)
