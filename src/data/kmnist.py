...
class KMNISTDataModule(DataModule):
    def __init__(self, data_dir: str, train_size: int, val_size: int, extra_size: int, batch_size: int,
        ...
        self.supervised_transform = transforms.Compose(
            [transforms.ToTensor(), transforms.Normalize((0.1917,), (0.3483,))])
        self.tensor_transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize((0.1917,), (0.3483,))])
    ...
    def prepare_data(self):
        ...
        if not self.train_data:
            MyKMNIST(self.data_dir, train=True, download=True)
            MyKMNIST(self.data_dir, train=False, download=True)

    def setup(self, stage: Optional[str] = None):
        if not self.train_data:
            full_data = MyKMNIST(self.data_dir, train=True)
            ...
            test_data = MyKMNIST(self.data_dir, train=False)
            self.test_data = AugmentedDataset(test_data, np.arange(len(test_data)), 0)
            predict_data = MyKMNIST(self.data_dir, train=False)
...
datasets.register_builder("kmnist", KMNISTDataModule)
