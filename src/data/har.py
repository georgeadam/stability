...
class HARDataModule(DataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        if not self.train_data:
            x, y = fetch_openml(data_id=1478, data_home=self.data_dir, return_X_y=True, as_frame=True)
            y = pd.factorize(y)[0]
            x = x.to_numpy()
            x = process_data(x)
            x = x.astype("float32")
...
datasets.register_builder("har", HARDataModule)
