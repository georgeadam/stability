...
class EMNISTDataModule(DataModule):
    def __init__(self, data_dir: str, train_size: int, val_size: int, extra_size: int, batch_size: int,
        ...
        self.tensor_transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize((0.1722,), (0.3309,))])
    ...
    def prepare_data(self):
        ...
        if not self.train_data:
            MyEMNIST(self.data_dir, split="letters", train=True, download=True)
            MyEMNIST(self.data_dir, split="letters", train=False, download=True)

    def setup(self, stage: Optional[str] = None):
        if not self.train_data:
            full_data = MyEMNIST(self.data_dir, split="letters", train=True)
            ...
            test_data = MyEMNIST(self.data_dir, split="letters", train=False)
            self.test_data = AugmentedDataset(test_data, np.arange(len(test_data)), 0)
            predict_data = MyEMNIST(self.data_dir, split="letters", train=False)
